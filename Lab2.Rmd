...
dim(data.rma)
MAplot(data.rma, pairs = TRUE, which=c(1,2,3,4), plot.method = "smoothScatter") # can change which to include which samples you want to use
...
```{r, eval = FALSE}
# Convert gene names
Annot <- data.frame(REFSEQ=sapply(names(contents(drosophila2REFSEQ)), paste, collapse=", "),
...
Alternatively, use a modeling approach:
```{r, eval = FALSE}
...
Two-factor design: (FYI: don't have more than 2 levels in the data, so you cannot actually run this)
```{r, eval = FALSE}
design = c("WT.U", "WT.S", "Mu.U", "Mu.S")
